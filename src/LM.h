...
class LM{
	public:
		...
		void knEstimate(bool interpolate);
		// void normalize();
		...
		void calculateDiscounts(vector<vector<vector<double> > >& discounts, bool logrized=false);
		void adEstimate(Node& node, vector<vector<vector<double> > >& discounts, int order, WordID e_i);
	...
	private:
		...
		void collectCountofCount(Node& node, vector<vector<double> >& coc, int order,bool logrized,WordID e_i);
